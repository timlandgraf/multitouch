...
public class ProxyServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest httpServletRequest,
		...
		debug("POST Request URL: " + httpServletRequest.getRequestURL(),
				"    Content Type: " + contentType, " Destination URL: "
						+ destinationUrl);
		...
		if (ServletFileUpload.isMultipartContent(httpServletRequest)) {
			...
			if (contentType == null
					|| PostMethod.FORM_URL_ENCODED_CONTENT_TYPE
							.equals(contentType)) {
				this.handleStandardPost(postMethodProxyRequest,
						httpServletRequest);
			} else {
				this.handleContentPost(postMethodProxyRequest,
						httpServletRequest);
			}
		}
		// Execute the proxy request
	...
	private void executeProxyRequest(HttpMethod httpMethodProxyRequest,
			...
			if (followRedirects) {
				if (stringLocation.contains("jsessionid")) {
					Cookie cookie = new Cookie("JSESSIONID",
							stringLocation.substring(stringLocation
									.indexOf("jsessionid=") + 11));
					cookie.setPath("/");
					httpServletResponse.addCookie(cookie);
					// debug("redirecting: set jessionid (" + cookie.getValue()
					// + ") cookie from URL");
				} else if (httpMethodProxyRequest
						.getResponseHeader("Set-Cookie") != null) {
					Header header = httpMethodProxyRequest
					...
					cookie.setPath("/");
					// debug("redirecting: setting cookie: " + cookie.getName()
					// + ":" + cookie.getValue() + " on " + cookie.getPath());
					httpServletResponse.addCookie(cookie);
				}
		...
		for (Header header : headerArrayResponse) {
			if (header.getName().equals("Transfer-Encoding")
					...
					|| header.getName().equals("Content-Encoding")
					&& header.getValue().equals("gzip") || // don't copy gzip
															// header
				...
				httpServletResponse.setHeader(header.getName(), header
						.getValue());
			}
		}
		...
		if (isBodyParameterGzipped(responseHeaders)) {
			debug("GZipped: true");
			if (!followRedirects
					&& intProxyResponseCode == HttpServletResponse.SC_MOVED_TEMPORARILY) {
				response = httpMethodProxyRequest.getResponseHeader(
						STRING_LOCATION_HEADER).getValue();
				httpServletResponse.setStatus(HttpServletResponse.SC_OK);
				intProxyResponseCode = HttpServletResponse.SC_OK;
				httpServletResponse.setHeader(STRING_LOCATION_HEADER, response);
			} else {
				response = new String(ungzip(httpMethodProxyRequest
						.getResponseBody()));
			}
		...
		debug("Received status code: " + intProxyResponseCode, "Response: "
				+ response);
	...
	private void handleContentPost(PostMethod postMethodProxyRequest,
		...
		if (contentType.startsWith("text/x-gwt-rpc")) {
			...
			String clientUrl = clientHost
					+ ((clientPort != 80) ? ":" + clientPort : "");
			String serverUrl = stringProxyHost
					+ ((intProxyPort != 80) ? ":" + intProxyPort : "")
					+ httpServletRequest.getServletPath();
			// debug("Replacing client (" + clientUrl + ") with server (" +
			// serverUrl + ")");
